...
bool is_declared(string s) 
{
	/*for (int i = 0; i < names.size(); ++i)
		if (names[i].name == s) return true;*/ //Warning hiba...
...
{
	if(is_declared(var)) error(var, " deklarálva van kétszer");
...
{
	/*for (int i = 0; i < names.size(); ++i)
		if (names[i].name == s) return names[i].value;*/ //Warning hiba (-Wall segítségével)
	for(const auto& v : names)
...
void set_value(string s, double d)
{
	/*for (int i = 0; i <= names.size(); ++i)
		if (names[i].name == s) {
			names[i].value = d;
			return;
		}*/  //Warning hiba (-Wall segítségével)
	for(auto& v : names)
		if(v.name == s)
		{
			v.value = d;
...
Token Token_stream::get()
{
	...
	default: // Nem szám vagy kifejezés
		...
		{                // Vagy olyan definíciót észlel, akkor azt a szabályt fogja alkalmazni
			...
			while (cin.get(ch) && (isalpha(ch) || isdigit(ch))) s+= ch; // s = ch helyett s+ = ch
			cin.unget();
...
double negyzetgyokfg(){ 
	...
	return sqrt(d); //sqrt(81)
}
double hatvanyozas(){ // Ezt úgy kell elképzelni, hogy pow(d,i) 
 
	Token t = ts.get(); // Beolvassuk a tokeneket pow(expression,expression)
...
{ //változók nevek, számok, -, +, sqrt,pow stb.    ((Number '(' ')' ))
	Token t = ts.get();
	switch (t.kind) 
	{
		case '(':
...
double term() // *,/, %-t csinálja  (( Term */% Primary ))
{
	double left = primary();
	while (true) {
		Token t = ts.get();
		...
		case '%': //Modulo hozzáadása (nem volt benne drillben)
		{
			double d = primary();           // Pl. 10%2
			if (d == 0) error ("%: divide by zero");
			left = fmod(left, d);
			t = ts.get();
			break;
...
double expression() // + és -   (( Expression '+' Term ))
{
	double left = term();
	while (true) {
		Token t = ts.get();
		switch (t.kind) {        //Pl. 50+2, (23-2)+3
		case '+':
			left += term();
			break;
		case '-':
...
{
	Token t = ts.get();
	if (t.kind != name) error("name expected in declaration"); // t.kind != 'a' ???? --> Javítva
	string name = t.name; 
	...
	double d = expression();
	//names.push_back(Variable(name, value)); // Hiba...Ha ezt megadjuk, akkor kiírja (2x deklarált..)
	define_name(name,d);
...
{
	while (cin) try { //Amíg igaz, míg nem nem szakítjuk //Hiba: while(true) --> while(cin)
